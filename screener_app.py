...
def run_gemini_fundamental_analysis(ticker_input, reports_store=None, status_log=None):
    ...
    headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/125.0.0.0 Safari/537.36', 'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,*/*;q=0.8', 'Referer': 'https://www.screener.in/'}
    ...
    try:
        ...
        if status_log: status_log.write(f"Discovered: {len(ars)} ARs, {len(transcripts)} Transcripts, {len(ppts)} PPTs.")
        ...
        def try_download(text, href):
            ...
            try:
                ...
                if status_log: status_log.write(f"  -> Skipped {text}: Download error - {e}")
            return False
        ...
        if not pdf_files:
            ...
            return None
        if status_log: status_log.write(f"🤖 **{clean_ticker}:** Uploading {len(pdf_files)} PDFs to Gemini...")
        uploaded_files = []
        ...
        for f in uploaded_files:
            while True:
                ...
                time.sleep(3)
        if status_log: status_log.write(f"Analyzing **{clean_ticker}** fundamentals...")
