...
# ==========================================
st.set_page_config(page_title="India Equities Screener & Watchlist Studio", page_icon="📈", layout="wide")
...
EPS_Q_ALIASES = ["earnings_per_share_diluted_yoy_growth_fq", "earnings_per_share_fq_yoy_growth", "earnings_per_share_diluted_yoy_growth_quarterly", "basic_eps_yoy_growth_fq"]
SALES_Q_ALIASES = ["revenue_yoy_growth_fq", "total_revenue_yoy_growth_fq", "revenue_yoy_growth_quarterly", "sales_yoy_growth_fq"]
...
def load_data(file_name, default_val):
    res = _gist_request("GET", file_name)
    if res: return res
...
def save_data(file_name, data):
    try:
        with open(file_name, "w") as f: json.dump(data, f, indent=2)
    except Exception: pass
    _gist_request("PATCH", file_name, data)

# ==========================================
# STATE INITIALIZATION
# ==========================================
if "watchlists" not in st.session_state: st.session_state.watchlists = load_data(WATCHLIST_FILE, {"Post Breakout Monitor": ["NSE:ZOMATO", "NSE:CDSL", "NSE:TITAGARH"], "Focus List": ["NSE:JINDWORLD", "NSE:TRENT", "NSE:HAL", "NSE:RECLTD"], "Weekly Focus": ["NSE:BHEL", "NSE:ABB", "NSE:SIEMENS", "NSE:CGPOWER"], "Scan Bulk": [], "Sold Stocks": []})
...
# ==========================================
# TRADINGVIEW API & CORE DATA FUNCTIONS
# ==========================================
def coalesce_columns(df, col_list):
    res = pd.Series(index=df.index, dtype="float64")
    for c in col_list:
        if c in df.columns: res = res.fillna(pd.to_numeric(df[c], errors="coerce"))
    return res
...
def clean_tv_date_col(val_series):
    ...
    dt_combined = dt_unix_s.fillna(dt_unix_ms).fillna(pd.to_datetime(val_series, errors="coerce"))
...
def add_clean_ipo_date_col(df):
    ...
    if ipo_cols:
        ...
        for c in ipo_cols: clean_dt_df[c] = clean_tv_date_col(df[c])
        df["IPO_Date_DT"] = clean_dt_df.max(axis=1)
    else: df["IPO_Date_DT"] = pd.NaT
...
def fetch_screener_data(exchanges, min_mcap, vol_period_days, ma_columns_to_fetch, limit_rows):
    if not exchanges: return pd.DataFrame()
    min_mcap_inr = min_mcap * 10_000_000
    tv_vol_col = f"average_volume_{vol_period_days}d_calc"
    select_cols = ["name", "close", "change", "high", "low", "open", "volume", "market_cap_basic", tv_vol_col, "ADR", "price_52_week_high", "price_52_week_low", "exchange", "type", "industry", "sector", "index", "ipo_offer_date", "offer_date", "recent_ipo_date", "ipo_date", "listing_date", "Perf.W", "Perf.1M", "Perf.3M", "Perf.6M", "Perf.YTD", "Perf.Y"] + EPS_Q_ALIASES + SALES_Q_ALIASES
    for c in ma_columns_to_fetch:
        if c not in select_cols: select_cols.append(c)
    q = Query().set_markets("india").select(*select_cols).where(col("market_cap_basic") >= min_mcap_inr).order_by("volume", ascending=False).limit(limit_rows)
    try:
...
def fetch_watchlist_enrichMENT(symbol_list):
    if not symbol_list: return pd.DataFrame()
    bare_names = [s.split(":")[-1].strip().upper() for s in symbol_list]
    select_cols = ["name", "close", "change", "ADR", "market_cap_basic", "exchange", "industry", "sector", "index", "ipo_offer_date", "offer_date", "recent_ipo_date", "ipo_date", "listing_date", "Perf.W", "Perf.1M", "Perf.3M", "Perf.6M", "Perf.YTD", "Perf.Y"] + EPS_Q_ALIASES + SALES_Q_ALIASES
    ...
    try:
        ...
        if not df.empty:
            ...
            for p_col in ["Perf.W", "Perf.1M", "Perf.3M", "Perf.6M", "Perf.YTD", "Perf.Y"]:
                if p_col in df.columns: df[p_col.replace(".", " % ").replace("W", "1W").replace("Y", "1Y")] = pd.to_numeric(df[p_col], errors="coerce").round(2)
            df = df.drop_duplicates(subset=["name"], keep="first")
            df["Sector"] = df.apply(lambda r: map_to_indian_classification(r.get("industry", ""), r.get("sector", ""))[0], axis=1)
            df["Industry"] = df.apply(lambda r: map_to_indian_classification(r.get("industry", ""), r.get("sector", ""))[1], axis=1)
        return df
...
# ==========================================
# REPORT PDF EXPORT & GEMINI AI ENGINE
# ==========================================
...
- **🚀 PRIMARY CATALYST / BREAKOUT TRIGGER:** [1-2 BOLD sentences. If NONE, state: "⚠️ NO CLEAR FORWARD CATALYST DETECTED"].
...
"""
        ...
        if reports_store is not None:
            ...
            save_data(REPORTS_FILE, reports_store)
        else:
            st.session_state.fundamental_reports[clean_ticker] = report_entry
            save_data(REPORTS_FILE, st.session_state.fundamental_reports)
...
"""
    try:
        ...
        save_data(BRIEFINGS_FILE, st.session_state.market_briefings)
        return entry
    except Exception as e:
...
# ==========================================
# EXCEL COLOR SCALE GRADIENT ENGINE
# ==========================================
...
def get_wl_dots(symbol, watchlists_dict):
    ...
    for wl_name, sym_list in watchlists_dict.items():
        ...
        if bare_sym in wl_bare_symbols:
            ...
            dot = "🔵" if "breakout" in name_lower else "🟢" if "focus" in name_lower else "🟡" if "weekly" in name_lower else "🟠" if "bulk" in name_lower else "🔴" if "sold" in name_lower else "🟣"
            if dot not in dots: dots.append(dot)
    return "".join(dots)

def is_circuit_stock_badge(row, bands_map):
    sym = str(row.get("name", "")).replace("🚨", "").strip().upper()
    if bands_map.get(sym, "") in ["2", "5", "10"]: return True
    high, low, open_p, close_p, change_p = [pd.to_numeric(row.get(k), errors="coerce") for k in ["high", "low", "open", "close", "change"]]
    change_p = abs(change_p)
    if pd.notna(high) and pd.notna(low) and high == low and high > 0 and pd.notna(change_p) and change_p > 1.5: return True
    if pd.notna(close_p) and pd.notna(high) and pd.notna(low) and (close_p == high or close_p == low) and (high != open_p):
        if (1.97 <= change_p <= 2.00) or (4.97 <= change_p <= 5.00) or (9.97 <= change_p <= 10.00): return True
    return False
...
def cb_move_top(wl_name, sym):
    if sym in st.session_state.watchlists.get(wl_name, []):
        st.session_state.watchlists[wl_name].remove(sym)
        st.session_state.watchlists[wl_name].insert(0, sym)
        save_data(WATCHLIST_FILE, st.session_state.watchlists)

def cb_move_up(wl_name, sym):
    if sym in st.session_state.watchlists.get(wl_name, []):
        idx = st.session_state.watchlists[wl_name].index(sym)
        if idx > 0:
            st.session_state.watchlists[wl_name].pop(idx)
            st.session_state.watchlists[wl_name].insert(idx - 1, sym)
            save_data(WATCHLIST_FILE, st.session_state.watchlists)

def cb_move_down(wl_name, sym):
    if sym in st.session_state.watchlists.get(wl_name, []):
        idx = st.session_state.watchlists[wl_name].index(sym)
        if idx < len(st.session_state.watchlists[wl_name]) - 1:
            st.session_state.watchlists[wl_name].pop(idx)
            st.session_state.watchlists[wl_name].insert(idx + 1, sym)
            save_data(WATCHLIST_FILE, st.session_state.watchlists)
...
def cb_move_bottom(wl_name, sym):
    if sym in st.session_state.watchlists.get(wl_name, []):
        st.session_state.watchlists[wl_name].remove(sym)
        st.session_state.watchlists[wl_name].append(sym)
        save_data(WATCHLIST_FILE, st.session_state.watchlists)

def cb_jump_rank(wl_name, sym, target_rank):
    if sym in st.session_state.watchlists.get(wl_name, []):
        st.session_state.watchlists[wl_name].remove(sym)
        new_idx = max(0, min(len(st.session_state.watchlists[wl_name]), target_rank - 1))
        st.session_state.watchlists[wl_name].insert(new_idx, sym)
        save_data(WATCHLIST_FILE, st.session_state.watchlists)

# ==========================================
# DICTIONARIES (Minified)
# ==========================================
INDIAN_SECTOR_HIERARCHY = {"Automobile and Auto Components": ["Automobiles", "Auto Components & Ancillaries", "Tyres & Rubber"], "Capital Goods": ["Aerospace & Defense", "Electrical Equipment", "Engineering Services", "Industrial Manufacturing", "Industrial Products"], "Chemicals": ["Chemicals & Petrochemicals", "Fertilizers & Agrochemicals"], "Construction": ["Civil Construction", "Infrastructure Developers"], "Construction Materials": ["Cement & Cement Products", "Ceramics & Building Materials"], "Consumer Durables": ["Consumer Electronics & Appliances", "Gems, Jewellery & Watches", "Household & Personal Products"], "Consumer Services": ["Leisure Services", "Restaurants & QSR", "Retailing", "Travel & Tourism"], "Diversified": ["Diversified Commercial Services", "Diversified Industrials"], "Fast Moving Consumer Goods": ["Agricultural Food & Other Products", "Beverages", "Food Products", "Personal Care", "Tobacco Products"], "Financial Services": ["Asset Management", "Banks", "Capital Markets", "Finance & NBFCs", "Financial Technology (Fintech)", "Insurance"], "Forest Materials": ["Paper, Forest & Jute Products"], "Healthcare": ["Healthcare Research, Analytics & Technology", "Healthcare Services", "Medical Equipment & Supplies", "Pharmaceuticals & Biotechnology"], "Information Technology": ["IT - Hardware", "IT - Software & Consulting", "IT - Services"], "Media, Entertainment & Publication": ["Broadcasting & Cable TV", "Entertainment & Content", "Print Media & Publishing"], "Metals & Mining": ["Ferrous Metals (Steel & Iron)", "Non-Ferrous Metals", "Minerals & Mining"], "Oil, Gas & Consumable Fuels": ["Consumable Fuels & Coal", "Oil & Gas Exploration & Production", "Petroleum Products & Refining"], "Power": ["Power Generation", "Power Transmission & Distribution"], "Realty": ["Real Estate Developers", "Real Estate Services"], "Services": ["Commercial & Professional Services", "Logistics & Transportation Services", "Port & Shipping Services"], "Telecommunication": ["Telecom - Equipment & Accessories", "Telecom - Services"], "Textiles": ["Garments & Apparels", "Textiles & Weaving"], "Utilities": ["Gas Transmission & Utilities", "Water & Other Utilities"]}
TV_TO_INDIAN_MAP = {("Commercial Services", "Financial Publishing/Services"): ("Financial Services", "Capital Markets"), ("Commercial Services", "Miscellaneous Commercial Services"): ("Services", "Commercial & Professional Services"), ("Commercial Services", "Personnel Services"): ("Services", "Commercial & Professional Services"), ("Consumer Durables", "Automotive Aftermarket"): ("Automobile and Auto Components", "Auto Components & Ancillaries"), ("Consumer Durables", "Electronics/Appliances"): ("Consumer Durables", "Consumer Electronics & Appliances"), ("Consumer Durables", "Home Furnishings"): ("Consumer Durables", "Household & Personal Products"), ("Consumer Durables", "Homebuilding"): ("Realty", "Real Estate Developers"), ("Consumer Durables", "Motor Vehicles"): ("Automobile and Auto Components", "Automobiles"), ("Consumer Durables", "Other Consumer Specialties"): ("Consumer Durables", "Gems, Jewellery & Watches"), ("Consumer Non-Durables", "Apparel/Footwear"): ("Textiles", "Garments & Apparels"), ("Consumer Non-Durables", "Beverages: Alcoholic"): ("Fast Moving Consumer Goods", "Beverages"), ("Consumer Non-Durables", "Food: Major Diversified"): ("Fast Moving Consumer Goods", "Food Products"), ("Consumer Non-Durables", "Food: Specialty/Candy"): ("Fast Moving Consumer Goods", "Food Products"), ("Consumer Non-Durables", "Household/Personal Care"): ("Fast Moving Consumer Goods", "Personal Care"), ("Consumer Services", "Broadcasting"): ("Media, Entertainment & Publication", "Broadcasting & Cable TV"), ("Consumer Services", "Hotels/Resorts/Cruise lines"): ("Consumer Services", "Travel & Tourism"), ("Consumer Services", "Movies/Entertainment"): ("Media, Entertainment & Publication", "Entertainment & Content"), ("Consumer Services", "Publishing: Books/Magazines"): ("Media, Entertainment & Publication", "Print Media & Publishing"), ("Consumer Services", "Restaurants"): ("Consumer Services", "Restaurants & QSR"), ("Distribution Services", "Electronics Distributors"): ("Capital Goods", "Industrial Products"), ("Distribution Services", "Medical Distributors"): ("Healthcare", "Medical Equipment & Supplies"), ("Distribution Services", "Wholesale Distributors"): ("Services", "Commercial & Professional Services"), ("Electronic Technology", "Aerospace & Defense"): ("Capital Goods", "Aerospace & Defense"), ("Electronic Technology", "Computer Communications"): ("Telecommunication", "Telecom - Equipment & Accessories"), ("Electronic Technology", "Computer Peripherals"): ("Information Technology", "IT - Hardware"), ("Electronic Technology", "Electronic Components"): ("Capital Goods", "Electrical Equipment"), ("Electronic Technology", "Electronic Equipment/Instruments"): ("Capital Goods", "Electrical Equipment"), ("Electronic Technology", "Electronic Production Equipment"): ("Capital Goods", "Industrial Manufacturing"), ("Electronic Technology", "Telecommunications Equipment"): ("Telecommunication", "Telecom - Equipment & Accessories"), ("Energy Minerals", "Oil & Gas Production"): ("Oil, Gas & Consumable Fuels", "Oil & Gas Exploration & Production"), ("Energy Minerals", "Oil Refining/Marketing"): ("Oil, Gas & Consumable Fuels", "Petroleum Products & Refining"), ("Finance", "Finance/Rental/Leasing"): ("Financial Services", "Finance & NBFCs"), ("Finance", "Financial Conglomerates"): ("Financial Services", "Finance & NBFCs"), ("Finance", "Investment Banks/Brokers"): ("Financial Services", "Capital Markets"), ("Finance", "Investment Managers"): ("Financial Services", "Asset Management"), ("Finance", "Life/Health Insurance"): ("Financial Services", "Insurance"), ("Finance", "Major Banks"): ("Financial Services", "Banks"), ("Finance", "Multi-Line Insurance"): ("Financial Services", "Insurance"), ("Finance", "Real Estate Development"): ("Realty", "Real Estate Developers"), ("Finance", "Regional Banks"): ("Financial Services", "Banks"), ("Health Services", "Hospital/Nursing Management"): ("Healthcare", "Healthcare Services"), ("Health Services", "Medical/Nursing Services"): ("Healthcare", "Healthcare Services"), ("Health Technology", "Biotechnology"): ("Healthcare", "Pharmaceuticals & Biotechnology"), ("Health Technology", "Medical Specialties"): ("Healthcare", "Medical Equipment & Supplies"), ("Health Technology", "Pharmaceuticals: Generic"): ("Healthcare", "Pharmaceuticals & Biotechnology"), ("Health Technology", "Pharmaceuticals: Major"): ("Healthcare", "Pharmaceuticals & Biotechnology"), ("Health Technology", "Pharmaceuticals: Other"): ("Healthcare", "Pharmaceuticals & Biotechnology"), ("Industrial Services", "Contract Drilling"): ("Oil, Gas & Consumable Fuels", "Oil & Gas Exploration & Production"), ("Industrial Services", "Engineering & Construction"): ("Construction", "Civil Construction"), ("Industrial Services", "Oilfield Services/Equipment"): ("Oil, Gas & Consumable Fuels", "Oil & Gas Exploration & Production"), ("Non-Energy Minerals", "Construction Materials"): ("Construction Materials", "Ceramics & Building Materials"), ("Non-Energy Minerals", "Forest Products"): ("Forest Materials", "Paper, Forest & Jute Products"), ("Non-Energy Minerals", "Other Metals/Minerals"): ("Metals & Mining", "Minerals & Mining"), ("Non-Energy Minerals", "Steel"): ("Metals & Mining", "Ferrous Metals (Steel & Iron)"), ("Process Industries", "Agricultural Commodities/Milling"): ("Fast Moving Consumer Goods", "Agricultural Food & Other Products"), ("Process Industries", "Chemicals: Agricultural"): ("Chemicals", "Fertilizers & Agrochemicals"), ("Process Industries", "Chemicals: Major Diversified"): ("Chemicals", "Chemicals & Petrochemicals"), ("Process Industries", "Chemicals: Specialty"): ("Chemicals", "Chemicals & Petrochemicals"), ("Process Industries", "Containers/Packaging"): ("Capital Goods", "Industrial Products"), ("Process Industries", "Industrial Specialties"): ("Capital Goods", "Industrial Manufacturing"), ("Process Industries", "Pulp & Paper"): ("Forest Materials", "Paper, Forest & Jute Products"), ("Process Industries", "Textiles"): ("Textiles", "Textiles & Weaving"), ("Producer Manufacturing", "Auto Parts: OEM"): ("Automobile and Auto Components", "Auto Components & Ancillaries"), ("Producer Manufacturing", "Building Products"): ("Construction Materials", "Cement & Cement Products"), ("Producer Manufacturing", "Electrical Products"): ("Capital Goods", "Electrical Equipment"), ("Producer Manufacturing", "Industrial Machinery"): ("Capital Goods", "Industrial Manufacturing"), ("Producer Manufacturing", "Metal Fabrication"): ("Capital Goods", "Industrial Manufacturing"), ("Producer Manufacturing", "Miscellaneous Manufacturing"): ("Capital Goods", "Industrial Products"), ("Producer Manufacturing", "Office Equipment/Supplies"): ("Consumer Durables", "Household & Personal Products"), ("Producer Manufacturing", "Trucks/Construction/Farm Machinery"): ("Automobile and Auto Components", "Automobiles"), ("Retail Trade", "Apparel/Footwear Retail"): ("Consumer Services", "Retailing"), ("Retail Trade", "Electronics/Appliance Stores"): ("Consumer Services", "Retailing"), ("Retail Trade", "Internet Retail"): ("Consumer Services", "Retailing"), ("Retail Trade", "Specialty Stores"): ("Consumer Services", "Retailing"), ("Technology Services", "Information Technology Services"): ("Information Technology", "IT - Services"), ("Technology Services", "Internet Software/Services"): ("Information Technology", "IT - Software & Consulting"), ("Technology Services", "Packaged Software"): ("Information Technology", "IT - Software & Consulting"), ("Transportation", "Air Freight/Couriers"): ("Services", "Logistics & Transportation Services"), ("Transportation", "Airlines"): ("Consumer Services", "Travel & Tourism"), ("Transportation", "Marine Shipping"): ("Services", "Port & Shipping Services"), ("Transportation", "Other Transportation"): ("Services", "Logistics & Transportation Services"), ("Transportation", "Railroads"): ("Services", "Logistics & Transportation Services"), ("Utilities", "Electric Utilities"): ("Power", "Power Generation"), ("Utilities", "Gas Distributors"): ("Utilities", "Gas Transmission & Utilities")}
...
with tab_market_health:
  ...
  with tab_sector_heat:
    if not df_heat.empty: st.table(style_sector_heatmap(df_heat))
    else: st.info("Sector Heatmap data not available yet.")

  with tab_sector_rot:
    if not df_rot.empty: st.table(style_rotation_tracker(df_rot))
