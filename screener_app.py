...
# ==========================================
# UNSTOPPABLE SINGLETON BACKGROUND AI WORKER TRACKER (@st.cache_resource)
# ==========================================
@st.cache_resource
def get_ai_worker_state():
  return {
      "is_running": False,
      "total": 0,
      "completed": 0,
...
AI_WORKER_STATE = get_ai_worker_state()


def background_ai_worker(
    symbols_list, force_reanalyze, auth_config, reports_dict
):
  AI_WORKER_STATE["is_running"] = True
  AI_WORKER_STATE["total"] = len(symbols_list)
  AI_WORKER_STATE["completed"] = 0
  AI_WORKER_STATE["logs"] = []

  for idx, sym in enumerate(symbols_list):
    clean_sym = sym.split(":")[-1].strip().upper()
    AI_WORKER_STATE["current_ticker"] = clean_sym
    AI_WORKER_STATE["message"] = (
        f"Analyzing ({idx + 1}/{len(symbols_list)}): {clean_sym} — Downloading"
        " PDFs & Running Gemini..."
    )

    if clean_sym in reports_dict and not force_reanalyze:
      AI_WORKER_STATE["logs"].append(
          f"⏩ [{idx + 1}/{len(symbols_list)}] {clean_sym}: Skipped (Report"
          " already exists in Gist)."
      )
      AI_WORKER_STATE["completed"] += 1
      continue

    try:
      AI_WORKER_STATE["logs"].append(
          f"📡 [{idx + 1}/{len(symbols_list)}] {clean_sym}: Downloading"
          " Screener.in PDFs..."
      )
      ...
      if new_rep:
        ...
        AI_WORKER_STATE["logs"].append(
            f"✅ [{idx + 1}/{len(symbols_list)}] {clean_sym}: Complete! ->"
            f" {new_rep.get('verdict')} (Saved to Gist)"
        )
      else:
        AI_WORKER_STATE["logs"].append(
            f"⚠️ [{idx + 1}/{len(symbols_list)}] {clean_sym}: No PDFs found or"
            " Screener page inaccessible."
        )
    except Exception as e:
      AI_WORKER_STATE["logs"].append(
          f"❌ [{idx + 1}/{len(symbols_list)}] {clean_sym}: Error -> {e}"
      )

    AI_WORKER_STATE["completed"] += 1
    AI_WORKER_STATE["last_updated"] = time.time()

  AI_WORKER_STATE["is_running"] = False
  AI_WORKER_STATE["message"] = (
    ...
    if df.empty:
      ...
      if GLOBAL_AI_WORKER_STATE["is_running"]:
        ...
        with st.expander("📜 Live Worker Execution Logs", expanded=True):
          for log_entry in GLOBAL_AI_WORKER_STATE["logs"][-8:]:
            st.write(log_entry)
        if st.button("🔄 Refresh Progress & Table", key=f"ref_worker_{rc}_{sc}"):
          st.rerun()
      elif (
          GLOBAL_AI_WORKER_STATE["message"]
          and time.time() - GLOBAL_AI_WORKER_STATE["last_updated"] < 120
      ):
        st.success(GLOBAL_AI_WORKER_STATE["message"])
        with st.expander("📜 Latest Worker Execution Logs", expanded=False):
          for log_entry in GLOBAL_AI_WORKER_STATE["logs"]:
            st.write(log_entry)
