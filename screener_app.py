...
# ==========================================
# PDF GENERATOR ENGINE FOR REPORTS
# ==========================================
def create_pdf_bytes(ticker, report_md):
  ...
  class PDF(FPDF):
    ...
    def header(self):
      ...
      self.cell(
          0,
          8,
          f"MINERVINI FUNDAMENTAL AI REPORT — {ticker}",
          ln=True,
          align="R",
      )
  ...
  for emoji_char, sub_text in emoji_replacements.items():
    text_clean = text_clean.replace(emoji_char, sub_text)
  ...
  pdf.cell(0, 8, f"{ticker} Fundamental Analysis Report", ln=True, align="L")
  pdf.ln(2)

  pdf.set_font("Helvetica", size=9)
  for line in text_clean.split("\n"):
    line_str = line.encode("latin-1", "replace").decode("latin-1").strip()
    if not line_str:
      pdf.ln(1.5)
      continue
